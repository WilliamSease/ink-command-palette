...
import {
	...
	macroReducer,
} from './state/macroReducer.js';
import {
	getNaviReducerinitialState,
	naviReducer,
...
export default function App() {
	...
	const [macros, macroDispatch] = useReducer(
		macroReducer,
		getMacroReducerinitialState(),
	);
	const [navi, naviDispatch] = useReducer(
		naviReducer,
		getNaviReducerinitialState(),
	);
	...
	return stdout.getWindowSize()[0] <= 65 ? (
		...
		<Box flexDirection="column" height={15} borderStyle={'classic'}>
			{navi.configOpen ? (
				...
				<EditMacro
					config={config}
					macros={[macros, macroDispatch]}
					navi={[navi, naviDispatch]}
				/>
			) : (
