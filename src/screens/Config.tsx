...
export const Config = (props: IProps) => {
            ...
            switch (actions[field]) {
                ...
                case 'writeOut':
                    if (write === 'rdy') {
                        ...
                        fs.writeFile('./storage/config.json', JSON.stringify(configState,null,2), (err) => {
                            if (err) {
                                setWrite('err')
                                setTimeout(() => setWrite('rdy'), 3000)
                            } else {
                                setWrite('ok')
                                setTimeout(() => naviDispatch({ type: 'toggleConfigOpen' }), 750)
                            }
                        });
                    }
                    break;
    ...
    return <Fragment>
        <Text color={configState.colorMap.main}> {`// Editing config \\\\`} </Text>
        <Text> </Text>
        <Box><Selector field={actions[field]} active={actions[0]} /><Text>Main Color (Enter): </Text><Text color={configState.colorMap.main}>{configState.colorMap.main}</Text></Box>
        <Text> </Text>
