						...
						fs.writeFile(
							...
							err => {
								if (err) {
									...
									setTimeout(
										() =>
											naviDispatch({type: 'setEditMacro', payload: undefined}),
										750,
									);
								}
							},
	...
	return (
		<Fragment>
			...
			<Box>
				...
				<Text>Command: </Text>
				<TextInput
					focus={actions[field] === actions[1]}
					value={activeMacro?.command ?? ''}
					onChange={val =>
						macroDispatch({
							type: 'editMacro',
							payload: {
